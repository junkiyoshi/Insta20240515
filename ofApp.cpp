...
void ofApp::update() {
	...
	for (int i = this->log_list.size() - 1; i > -1; i--) {
		...
		auto deg = ofMap(ofNoise(glm::vec3(location * 0.025, ofGetFrameNum() * 0.01)), 0, 1, -90, 90);
		feature += glm::vec2(45 * cos(deg * DEG_TO_RAD), 45 * sin(deg * DEG_TO_RAD));
